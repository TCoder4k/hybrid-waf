...
Phase 11 — Evaluation (complete). This was the last phase on the `docs/CLAUDE.md` §17 roadmap. Since then, two standalone, explicitly-scoped tasks (not numbered phases) were completed: the Admin Dashboard UI Redesign, and the full Security Events page — see "Dashboard UI Redesign detail" and "Security Events Page detail" below.
...
- Confirmed empirically (via Nest's route-registration log) that `AppController`'s `/health` maps _before_ `WafController`'s `{/*, ALL}` wildcard, so the proxy doesn't swallow the backend's own health endpoint — this was a real risk with a wildcard catch-all in a separate module, not just a theoretical one.
...
_(Superseded in Phase 4: the forwarding logic described above as living in `WafService.forward()` was relocated to `ProtectedApiClientService.forward()`; `WafService` is now the pipeline orchestrator. See "Phase 4 detail" below.)_
...
- Benign (`id=1`) → rule `NORMAL`, ML `SQL_INJECTION` at **0.60 confidence** (below the 0.7 threshold) → **ALLOW**, reason correctly names the sub-threshold ML flag. This is a genuine, honest finding: the model has some false-positive pull on plain benign input, caught here specifically _because_ the confidence gate exists — worth keeping in mind for Phase 11's evaluation writeup, not something to paper over.
...
- **DB-down live test:** stopped the throwaway Postgres — `POST /auth/login` → `503`, `GET /admin/events` with an already-issued (still cryptographically valid) token → `503`, and — critically — sending the SQLi payload through the WAF _still_ returned `403` (the BLOCK decision and its logging-failure-is-swallowed behavior from Phase 8 are completely independent of the Admin API's own DB error handling).

**Cleanup:** backend/protected-api/ml-service processes killed, throwaway Postgres container removed, no `.env` files left, nothing committed.

## Phase 9A detail

**Why this phase exists:** the SRS's Dashboard needs "Total Requests" and "Allowed Requests," which `SecurityEvent` (BLOCK-only, ADR-3) cannot supply — `COUNT(security_events)` would only ever show blocked traffic, misrepresented as a total. The user explicitly required a small, separate architecture-adjustment phase — designed and approved _before_ implementation — rather than letting Claude quietly bolt this onto Phase 10.

**Approval process (explicit user instruction):** a plan-only turn was run first (via plan mode) proposing the `TrafficMetric` design; the user approved it **with one required change** — the original proposal had the metrics write `await`ed inline (like `SecurityEventLogger`), and the user correctly pushed back that this would add a DB round-trip's latency to _every_ request including the high-volume ALLOW path, for a statistic the Dashboard can tolerate being slightly delayed. The approved, implemented design is fire-and-forget instead. This distinction (await-and-swallow vs. fire-and-forget-with-`.catch()`) is the single most important design decision in this phase — see below.
...
- **Deliberately not duplicated:** `/admin/stats` returns only the five `TrafficMetric` counters. "Recent Security Events" is served by the _existing_ `GET /admin/events?page=1&pageSize=10` (already ordered `timestamp: 'desc'` since Phase 9, so page 1 is already "most recent") — the Dashboard makes two calls rather than `/admin/stats` growing a second, duplicate query path over `SecurityEvent`. "Attack Distribution" is derived client-side from `sqlInjectionBlocks`/`xssBlocks` — no extra field, no extra query.
...
**No CI/CD existed at first.** `git push` only updated GitHub — nothing on the VPS watched the remote or redeployed automatically (confirmed by checking the repo for `.github/workflows/`, webhooks, or a watchtower-style setup: none existed). Every deploy was a manual SSH session running `git pull origin main && docker compose up -d --build` on the VPS. **CI/CD was added later this same session** — see the dedicated paragraph below.

**New `deploy.sh`** (repo root) — shorthand for that same manual sequence, run _on the VPS_ after SSHing in (`./deploy.sh`, `chmod +x` once after the first pull): logs the commit before/after pull, **aborts if the VPS working tree has uncommitted changes to tracked files** (a deploy target should never have local edits — `.env` files are untracked/gitignored so they don't trip this), then `git pull origin main`, `docker compose -f docker-compose.prod.yml up -d --build` (auto-detected, falls back to plain `docker compose` if that file is absent), `docker image prune -f` (avoid disk bloat from repeated rebuilds), and a final `docker compose ps`. Not CI/CD — still requires a human to SSH in and run it; that was explicitly out of scope for this ask.
...
**The safety check immediately caught a real problem.** The first `./deploy.sh` run on the VPS correctly aborted: the VPS's `docker-compose.yml` had uncommitted local edits — the production hardening from the original VPS setup session (loopback-only `backend`/`frontend` ports, no published port at all for `postgres`/`ml-service`, `restart: unless-stopped` on every service, `FRONTEND_URL`/`NEXT_PUBLIC_ADMIN_API_URL` pointing at the real domain instead of `localhost`) that had never been committed to Git — it only existed as a hand-edit on the server. A blind `git pull` would have silently reverted that hardening (republishing Postgres and the app ports to `0.0.0.0`, breaking CORS since `FRONTEND_URL` would revert to `localhost`) the next time anyone deployed.

**Fixed properly, not by force-pulling:** the user pasted the VPS's `git diff docker-compose.yml`. Rather than merge that into `docker-compose.yml` directly (would break every local-dev assumption documented throughout this project) or use a Compose override file (empirically verified via `docker compose config` that Compose **appends** list-valued keys like `ports` across `-f` files rather than replacing them — an override would leave the dev port published _alongside_ the loopback-only one, defeating the hardening), the fix is a new **`docker-compose.prod.yml`** — a full, standalone compose file (not a merge/override) capturing exactly what was hand-edited on the VPS, now version-controlled. `docker-compose.yml` (dev) is untouched. `deploy.sh` auto-detects and uses `docker-compose.prod.yml` when present. Documented properly for the first time in `docs/architecture.md` §20 (previously nothing about the VPS/Nginx/deploy setup existed in this file at all).

**Not attempted by Claude directly:** SSHing in and running the deploy — the session's local SSH key (`~/.ssh/id_ed25519`) was rejected by the VPS for every username tried (root, ubuntu, admin, duytu, deploy, waf, hybridwaf, tu); the user has separate working credentials (confirmed logging in themselves as `root`) that aren't available in this environment. The user ran `./deploy.sh` on the VPS themselves; hit one more real snag first — `git pull` refused because the VPS's own `chmod +x deploy.sh` had created a local file-mode diff (`100644` → `100755`) against the committed blob — fixed by staging that exact mode change locally (`git update-index --chmod=+x deploy.sh`) so a future commit carries `+x` in the blob itself and Linux checkouts never need to `chmod` it again. After `git checkout -- docker-compose.yml` (discarding the now-redundant hand-edit) + `git pull` + one more `chmod +x`, **`./deploy.sh` ran clean end-to-end**: all 5 containers rebuilt (cache-hit, no code had actually changed) and confirmed `Up`, correct `127.0.0.1:8090`/`127.0.0.1:8091` bindings, no other ports exposed.

**CI/CD added** (`.github/workflows/deploy.yml`) — the user asked for this explicitly after asking "is `deploy.sh` the same as CI/CD?" (it wasn't: `deploy.sh` is the _what_, CI/CD is the _automatic trigger_). 3 parallel CI jobs on every push to `main` (backend build+lint+`npm test`+`npm run test:e2e` against a real `postgres:16-alpine` Actions service container with `npx prisma migrate deploy` applied first — `database.e2e-spec.ts`/`app.e2e-spec.ts` need a genuinely reachable DB, unlike the fully-mocked `admin-auth.e2e-spec.ts`; frontend build+lint; ml-service `pytest` against the already-committed trained model), then a `deploy` job gated on all 3 (`needs:`) that SSHes in (`appleboy/ssh-action@v1.2.5`, version confirmed live via the GitHub releases page rather than assumed) and runs the exact same `cd /var/www/hybrid-waf && ./deploy.sh`. `actions/checkout@v7`/`actions/setup-node@v7`/`actions/setup-python@v7` versions likewise confirmed live, not from training-data memory. `concurrency: production-deploy` prevents two fast pushes from racing each other's rebuild on the VPS. **Auth method decided with the user, not assumed:** the VPS doesn't accept any SSH key for `root` at all (confirmed — only password auth worked when the user logged in manually), so `appleboy/ssh-action` is configured for password auth (`VPS_PASSWORD` secret), not the more conventional private-key input — asked via AskUserQuestion (password now for speed vs. provisioning a dedicated CI keypair now) and the user picked password. **Stated trade-off, not hidden:** this is full `root` access gated only by a password stored as an encrypted GitHub secret, not a newly-provisioned restricted deploy-only user — called out explicitly in `docs/architecture.md` §20 as a known follow-up rather than accepted silently. Requires the user to add repo secrets (`VPS_HOST`, `VPS_USERNAME`, `VPS_PASSWORD`, optionally `VPS_PORT`) in GitHub before the `deploy` job can succeed — not yet done as of this writing, workflow untested end-to-end (CI jobs themselves are ordinary GitHub-hosted runs and should just work; the SSH step is the untested part).

## Reverse-Proxy Generalization & L7 Protection initiative

**Trigger:** the user explicitly approved (2026-09-15) expanding scope beyond the original 20-day MVP boundary (`docs/CLAUDE.md` §2/§3) toward a production-oriented WAF that can protect an arbitrary configurable upstream and adds practical Layer-7 rate limiting/DoS protection — while being explicit that volumetric Layer-3/4 DDoS stays permanently out of scope. Full plan approved via plan mode; plan file: a 19-section architecture-audit report grounded in three parallel read-only codebase audits (backend WAF pipeline, ml-service, frontend dashboard).

**Audit findings that shaped the plan (evidence, not assumption):**

- `PROTECTED_API_URL` is already read per-request (not cached), a good swap-friendly pattern — but the _name_ "protected-api" is baked into Docker Compose service naming, the frontend's typed `SystemStatus.protectedApi` field, 2 UI labels, and several e2e test setups.
- Confirmed real proxy-transparency gaps: no timeout on the outbound upstream fetch, no multipart/form-data support, a body/Content-Type re-serialization mismatch (always `JSON.stringify`, regardless of original content type), silent redirect-following, non-binary-safe `.text()`-only response relay, no explicit body-size/header-timeout config.
- Confirmed no rate limiting exists anywhere (`@nestjs/throttler` absent, no custom code), no Docker resource limits on any service.
- **ml-service bottleneck confirmed by code, not just measurement:** `Dockerfile`'s `CMD` has no `--workers` (defaults to 1 uvicorn worker); `/predict` is a sync `def` handler doing CPU-bound scikit-learn inference (TF-IDF transform + LogisticRegression predict/predict_proba), GIL-serialized across all concurrent requests in that single process — directly explains the previously-measured local capacity ceiling (~150-210 req/s, p95 growing sharply past ~100 VUs).
- Dashboard already real (not placeholders) on `/system`/`/settings`/`/statistics` post the `linh`/`tung` branch merges — but **no DB-backed runtime-configurable settings exist anywhere** (`Admin`/`SecurityEvent`/`TrafficMetric` are the only Prisma models); everything configurable today is env-var only, and the Settings page's "Cấu hình phát hiện" card is read-only.
- `SecurityEvent.attackType`/`decision` are plain Prisma `String` fields, not enums — a new `RATE_LIMIT` classification needs zero schema migration, but 4 frontend spots (`EventsFilterBar`'s attack-type dropdown, `AttackDistributionChart`, `RecentActivityPanel`, `dashboard/page.tsx`'s percentage tiles) hardcode exactly `SQL_INJECTION`/`XSS` and would silently under-represent a third type rather than error.
- No SSRF/open-proxy risk exists today — confirmed the upstream target is exclusively env-config, never client-influenced, in every reachable code path.

**Key decisions locked into the plan:**

- Redis: **not introduced now** — the current deployment runs exactly one `backend` instance (confirmed via `docker-compose.prod.yml`), so in-memory rate limiting is correct, not just simpler, today. Deferred to optional Phase P4, conditioned strictly on the backend actually being horizontally scaled; when it does happen, fail-open with a local fallback (mirrors ADR-2's precedent), not fail-closed.
- Upstream rename: `UPSTREAM_URL` becomes canonical, with `PROTECTED_API_URL` kept as a one-phase (P1) fallback alias rather than a blind rename — avoids breaking existing deployments/tests mid-migration.
- ml-service `--workers N` fix kept as its own independently-approvable micro-phase (P3a) — this was already proposed earlier in this project's history and paused pending explicit approval; the plan preserves that as a standalone decision point rather than bundling it into the larger rate-limiting phase.
- DB-backed, dashboard-mutable settings (editing rate limits/ML threshold live) explicitly deferred to an optional Phase P9, not assumed part of this initiative — starts env-var-only, matching "smallest safe production-oriented implementation."

**Phases:** P0 (architecture+ADR+scope docs) → P1 (generic upstream) → P2 (proxy hardening, incl. the highest-risk raw-body/multipart refactor) → P3/P3a (rate limiting / ml-service workers) → P4 (Redis, conditional) → P5 (ML onboarding modes) → P6 (admin observability/settings) → P7 (load-test before/after) → P8 (deployment docs) → P9 (optional, DB-backed settings). Each phase follows the existing per-phase Definition of Done (`docs/CLAUDE.md` §16): build/lint/unit/e2e pass, `docs/memory.md` updated, no scope creep beyond that phase, user review, no Claude commit.

**Phase P0 detail (docs-only):**

- `docs/CLAUDE.md`: new §2a documenting the approved scope extension; §3 non-goals rewritten — "DDoS protection"/"Advanced rate limiting" replaced with a precise split (volumetric L3/4 DDoS stays a _permanent_ non-goal; L7 rate-limiting is now explicitly in scope per §2a); "SSRF detection" clarified as still meaning the traffic-inspection _feature_ (still a non-goal), distinct from the generic-upstream work's own SSRF-_safety_ engineering invariant (in scope, required).
- `docs/architecture.md`: new §21 (Generic Upstream), §22 (L7 Rate Limiting/DoS Protection), §23 (ML Onboarding Modes), each later updated from "Planned" to "Implemented" as the corresponding phase landed. ADR-8/ADR-9/ADR-10 added to the §19 log.
- No code, dependency, schema, Docker, commit, push, or deploy changes — docs only.

**User instruction mid-initiative:** after P0 and P1's report, the user explicitly said to stop gating phase-by-phase and implement everything else in one continuous pass, reviewing in the morning ("code 1 lượt hết chức năng luôn... sáng ra tôi check sau"). P2 through P8 below were built in that one session, each still verified with real build/lint/unit/e2e (and live manual checks for the genuinely new runtime behavior — rate limiting, WAF_MODE) before moving to the next, but without stopping for approval between them. Still no commit — that's the user's own step per `docs/CLAUDE.md` §14.

**Phase P1 detail — generic configurable upstream:**

- `backend/src/common/upstream-config.util.ts` (new): `resolveUpstreamUrl()` — `UPSTREAM_URL ?? PROTECTED_API_URL ?? 'http://localhost:3001'`, validated via `new URL(...)`, throws `InvalidUpstreamUrlError` on a malformed value. Single source of truth, used by both call sites below instead of each independently re-implementing the same fallback chain.
- `backend/src/modules/waf/protected-api-client.service.ts` → renamed **in place** `upstream-proxy.service.ts`, class `ProtectedApiClientService` → `UpstreamProxyService`. Kept inside `modules/waf/` rather than moving to a new `modules/upstream/` folder as the plan floated — the actual ask ("rename/refactor... update references") didn't require a structural move, and avoiding it kept the diff minimal (documented here as a deliberate deviation from the plan's `§8` suggestion, not an oversight).
- `SystemStatus`/`SystemComponentStatus` (`system-status.service.ts`, and the frontend's mirrored type in `lib/api.ts`): `protectedApi` field renamed to `upstream` — a coordinated backend+frontend change in the same pass, including `SystemStatusPanel.tsx`/`system/page.tsx` labels ("Protected API" → "Upstream").
- `main.ts`: boot-time `resolveUpstreamUrl()` call — logs and `process.exit(1)` on a malformed config, fails fast instead of 502ing every request forever. Live-verified both paths (malformed → exits with a clear message before touching the DB; well-formed → proceeds to normal Nest bootstrap).
- `docker-compose.yml`/`docker-compose.prod.yml`: `backend`'s `PROTECTED_API_URL` env key → `UPSTREAM_URL` (still pointing at the bundled `protected-api` by default — no behavior change for the existing dev/prod setup). `backend/.env.example` documents the new var + the alias.
- SSRF invariant: proven by dedicated tests, not just "by construction" — 4 new unit tests (spoofed Host/X-Forwarded-Host/body/query-string, all ignored) in `upstream-proxy.service.spec.ts`, plus one full-pipeline e2e test in `waf-proxy.e2e-spec.ts` (spoofed Host + X-Forwarded-Host through the real `WafController`→`WafService`→`UpstreamProxyService` chain, still lands on the configured fake upstream).
- **Real bug found and fixed during this phase, not by inspection:** the original `protected-api-client.service.spec.ts` (and `system-status.service.spec.ts`) restored an unset env var via `process.env.X = originalValue` where `originalValue` was `undefined` — Node coerces that assignment to the literal string `"undefined"` rather than deleting the key, silently leaking a bogus value into every test file that runs afterward in the same Jest worker process. This had been latent and harmless before (the old code's `?? 'default'` fallback only checks for `null`/`undefined`, and every consuming test mocked `fetch` anyway so the actual URL value passed through was never checked) — it only became an observable, failing bug once `resolveUpstreamUrl()` added strict `new URL()` validation, which throws on the string `"undefined"`. Fixed by a `restoreEnv()` helper (`delete` when the original was `undefined`, assign otherwise) in all three affected spec files.
- Verified: backend build/lint/unit (203/203 at this point)/e2e (53/53) all pass; frontend build/lint clean.

**Phase P2 detail — proxy hardening:**

- `UPSTREAM_TIMEOUT_MS` (default 10s, re-read per call like `resolveUpstreamUrl()`): `AbortController` around the outbound `fetch`, distinct `504 Gateway Timeout` vs the existing `502` (connection failure).
- `redirect: 'manual'` on the outbound fetch — a 3xx from upstream is now relayed to the client verbatim (status + `Location` + other headers) instead of the WAF silently following it. Verified live in Node that Node's `fetch` (undici) does **not** apply the browser opaque-redirect restriction here — `res.status`/`res.headers` are fully readable, confirmed by a quick standalone script before committing to this design.
- Binary-safe response relay: `response.text()` → `Buffer.from(await response.arrayBuffer())`. `ForwardedResponse.body` is now `string | Buffer` (`WafController`'s `res.send()` already handles both). New test proves a real non-UTF8 byte sequence round-trips unmangled.
- Content-type-aware outbound body (`backend/src/common/raw-body-capture.middleware.ts`, new): Nest's default `json`/`urlencoded` parsers only ever touch the stream for those two content types, leaving everything else (`multipart/form-data`, images, arbitrary binary, no body) completely unconsumed — this middleware captures those bytes into `req.rawBody`, is a no-op for json/urlencoded (already handled), and enforces the same byte-size cap with a `413` on overflow. `UpstreamProxyService` then builds the outbound body per content type: JSON re-`JSON.stringify`d from the already-parsed `req.body` (unchanged), urlencoded re-encoded via `URLSearchParams` (previously wrongly sent as JSON regardless of content type — a real transparency bug fixed here), everything else forwarded byte-for-byte from `req.rawBody`. Scope boundary stated explicitly in the code: `rawBody` is only ever used for forwarding, never read by `RequestNormalizerService`/detection — inspecting multipart/binary body contents for SQLi/XSS stays out of scope.
- `X-Forwarded-For`/`-Proto`/`-Host` added to the outbound request (previously absent) so a real upstream can see the actual client — `X-Forwarded-For` appends to any existing chain using the same `req.ip` already used for `SecurityEvent.sourceIp` (reused, not a second IP-resolution concept).
- `main.ts`: explicit `MAX_BODY_SIZE_BYTES` (default 1 MiB) via `app.useBodyParser('json'|'urlencoded', { limit })` (applies globally, `/admin/*` included, unchanged behavior otherwise) plus the raw-body middleware; `REQUEST_HEADERS_TIMEOUT_MS`/`REQUEST_TIMEOUT_MS` (defaults 15s/30s) set directly on the underlying `http.Server` returned by `app.listen()` — Node's own defaults (60s/300s) were unconfigured before, a real slowloris exposure.
- Verified: unit (217/217), e2e (53/53, including the pre-existing suite's fake-upstream behavior unaffected by the timeout/redirect/body changes), build/lint clean.

**Phase P3/P3a detail — L7 rate limiting + ml-service `--workers`:**

- New `backend/src/modules/rate-limit/` module: `token-bucket.ts` (pure, clock-injectable token-bucket, `sweep()` for bounding per-IP map memory under a real multi-IP attack), `concurrency-limiter.ts` (in-flight request cap), `ip-normalize.util.ts` (`::ffff:`-mapped IPv4 stripped before bucketing, so a dual-stack client can't get two buckets for one real IP), `rate-limit.config.ts` (env parsing: `RATE_LIMIT_PER_IP_RPS`/`_BURST`, `RATE_LIMIT_GLOBAL_RPS`/`_BURST`, `RATE_LIMIT_MAX_CONCURRENCY`), `rate-limit-recorder.service.ts` (records a rejection through the _same_ `SecurityEventLogger`/`TrafficMetricsRecorder` a real BLOCK uses, via a synthetic-but-clearly-labeled `DetectionResult`/`MLDetectionResult`/`DecisionResult` — fire-and-forget, not awaited, since a 429 under an actual flood must stay fast even if the DB is contended), `rate-limit.guard.ts` (`RateLimitGuard`, applied via `@UseGuards()` on `WafController` only — checks global bucket → per-IP bucket → concurrency, in that order, throws `HttpException(429)` with `Retry-After` staged via `res.setHeader()` before the throw so Nest's own response pipeline sends it correctly).
- `AttackClassification` (`common/types.ts`) gained `'RATE_LIMIT'`; `TrafficMetric` gained a `rateLimitBlocks` column (migration `20260915182549_add_rate_limit_blocks`, one `ALTER TABLE ... ADD COLUMN ... DEFAULT 0`) and `TrafficMetricsRecorder`/`TrafficMetricRepository` count it exactly like the existing `sqlInjectionBlocks`/`xssBlocks`.
- **Real cross-module DI bug found and fixed, not by inspection:** `@UseGuards(RateLimitGuard)` on `WafController` initially failed dependency resolution with "RateLimitRecorder... available in the WafModule module" — confirmed against this exact codebase's own `AuthModule`/`JwtAuthGuard` precedent (a comment already there explains it: "Nest resolves a class-referenced guard's own dependencies through the _consuming_ module's injector, not the guard's home module"). Fixed the same way `AuthModule` does — `RateLimitModule` now exports `RateLimitRecorder` alongside `RateLimitGuard`, not just the guard.
- ml-service `Dockerfile`: `CMD ["uvicorn", ...]` (JSON array, no shell, no env substitution) → shell-form `CMD uvicorn ... --workers ${WORKERS:-2}`, `ml-service/.env.example` documents `WORKERS` (set to the real host's core count). Live-verified: built the image, ran with `WORKERS=3`, confirmed 3 separate `Started server process [pid]`/`Application startup complete` lines in the container log.
- Live-verified end-to-end (not just unit-mocked): a real running backend with `RATE_LIMIT_PER_IP_BURST=3` returned 3× success then 429+`Retry-After: 1` on requests 4-6.
- Verified: unit (254/254 by this point), e2e (53/53), build/lint clean on both.

**Phase P5 detail — ML onboarding modes:**

- `backend/src/common/waf-mode.util.ts` (new): `resolveWafMode()` — `WAF_MODE` env var, one of `MONITOR`/`RULE_BLOCK_ML_MONITOR`/`HYBRID_BLOCK`, defaults to `HYBRID_BLOCK` (today's unchanged behavior) on anything unrecognized.
- `HybridDecisionEngine`: reads `this.mode` once at construction (same pattern already established for `ML_CONFIDENCE_THRESHOLD`) — `decide()` itself stays a pure function of its inputs plus that stored mode, unchanged signature. `MONITOR` suppresses both the rule-BLOCK and ML-confidence-BLOCK branches to `ALLOW`; `RULE_BLOCK_ML_MONITOR` suppresses only the ML-confidence branch (rule BLOCK unchanged). `DecisionResult` gained an optional `shadow: { wouldBlock: true, classification, reason }`, populated only when a suppressed branch would otherwise have fired — visible for free in `WafService`'s existing debug log line (`JSON.stringify(decision)` already includes it, no new logging code needed).
- Live-verified: a real SQLi payload (`?id=1 OR 1=1`) through a running backend with `WAF_MODE=MONITOR` correctly detected the rule match, ALLOWed instead of blocking, and logged the exact shadow decision (`"shadow":{"wouldBlock":true,"classification":"SQL_INJECTION",...}`) — confirmed via the debug log, not just a unit test.
- Verified: unit (252/252 at this point, 9 new WAF_MODE-specific cases), e2e (53/53), build/lint clean.

**Phase P6 detail — admin observability/settings:**

- `system-info.ts`/`SystemInfo` (backend) and its frontend mirror in `lib/api.ts`: gained `wafMode` (via `resolveWafMode()`) and `rateLimit` (via the same `loadRateLimitConfig()` the guard uses — one source of truth, not re-parsed). Live-verified via `GET /admin/system-info` returning real `wafMode`/`rateLimit` values matching the env the backend was started with.
- Settings page (`/settings`): new read-only "Giới hạn tần suất (Rate Limit)" card (per-IP/global rate+burst, max concurrency) alongside the existing "Cấu hình phát hiện" card, which now also shows the current `WAF_MODE`. Explicitly still read-only, env-sourced — no new mutation endpoint, matching the plan's "start env-var-only, DB-backed settings is a separate deferred feature (P9)" decision.
- The 4 frontend spots flagged in the P0 audit as "would silently under-represent a `RATE_LIMIT` classification" are all fixed: `EventsFilterBar`'s attack-type dropdown gained a "Rate Limit" option, `AttackDistributionChart` gained a third slice/color and an optional `rateLimitBlocks` prop, `RecentActivityPanel` gained a "Rate Limit" row, `dashboard/page.tsx` now passes `rateLimitBlocks` through to the chart. `RecentEventsTable`'s badge styling gained an explicit amber style for `RATE_LIMIT` (previously would have fallen back to the generic/unstyled default, which was safe but not polished).
- Not done, consistent with the plan: a new Statistics-page "Top blocked/rate-limited IPs" table — would need a new backend aggregate query, treated as a stretch item beyond the "don't silently misrepresent existing data" bar the other P6 items were held to; not built this pass.
- Verified: backend unit (254/254), e2e (53/53); frontend build/lint clean (TypeScript compiler confirms every `SystemInfo`/`TrafficStats` consumer was updated consistently).

**Phase P7 detail — load testing:**

- New `load-testing/rate-limit-test.js`: single VU burst (default 80 requests, no `sleep`) against a configured `RATE_LIMIT_PER_IP_BURST`, own metrics (`rate_limit_hit_count`, `rate_limit_retry_after_present`) kept deliberately separate from the capacity test's `http_req_failed` — a 429 here is success, not failure. Live-run against a real backend (`RATE_LIMIT_PER_IP_BURST=20`): 34/60 requests hit the limit, 100% of those carried `Retry-After`.
- Real before/after capacity comparison run locally (`load-testing/capacity-comparison-p3a.md`) at 20/50/100/200/400/800 VUs, full local stack (real trained ML model, real Postgres, real protected-api). Honest result, not cherry-picked: **100 VUs is the clean win** — p95 roughly halved (1.18s → 0.58s), throughput +39% (147 → 204 req/s), exactly the concurrency range the code-level root cause (single-worker, GIL-serialized ml-service) predicted. 200-400 VUs improved on p95 but not throughput, most likely because this "after" run shared the same 8-core dev machine with 4 ml-service worker processes now competing against everything else running in this long session — flagged as a confounded, non-isolated comparison rather than presented as a clean number. 800 VUs failed entirely at the network level in this run (`status: 0`, not a slow HTTP response) — traced to the backend still processing real requests in its own log during the failure window, so likely a local Docker-Desktop/WSL2 `host.docker.internal` connection-count limit in this specific sandboxed session, not a WAF/ml-service defect; not re-run repeatedly to chase an environment artifact.
- `load-testing/README.md` updated to document both new scripts and the capacity-vs-rate-limit metric separation.

**Phase P8 detail — deployment documentation:**

- New `docs/PROTECTING_ANOTHER_APP.md`: the SSRF invariant restated up front, both deployment topologies (same Docker network — no published port, exactly like `protected-api` already does; external upstream — firewall/egress-IP allowlisting is the operator's responsibility, Docker can't enforce it across hosts) with concrete compose snippets, the full new env-var reference (P1-P3's additions), the 3-stage `WAF_MODE` onboarding walkthrough (why, and that no restart is needed via `docker compose up -d`), post-cutover verification commands (curl-based, plus pointers to `rate-limit-test.js`/`k6-load-test.js`), a rollback section (upstream revert, loosen-not-toggle rate limits, drop back to `MONITOR`, the one migration is additive/safe), and an explicit "don't overclaim" closing section (no L3/4 DDoS mitigation, single-instance-only rate limiter, no dashboard-mutable settings yet).
- `docs/architecture.md` §21/§22/§23 updated from "Planned" to "Implemented," §19's ADR table note updated accordingly.

**Final full regression (after all of P1-P8, one last pass):** backend build clean, lint clean, unit **254/254** across 34 suites, e2e **53/53** across 5 suites (fresh throwaway Postgres, migrations applied cleanly including the new one); frontend build clean, lint clean (one pre-existing, unrelated warning in `lib/nav.ts` about an unused `TrendingUp` import — not touched by this initiative). `git status` confirms only the files listed above changed, no stray `.env`/secret files staged or touched, nothing committed.

**Admin Events RATE_LIMIT integration regression (2026-09-17):** The frontend P6 filter already sent `attackType=RATE_LIMIT`, and the rate limiter already persisted `SecurityEvent.attackType = RATE_LIMIT`, but `AdminController.parseListFilter()` still used the pre-P3 allow-list `{SQL_INJECTION, XSS}`. The request therefore returned `400 Bad Request` before `AdminService` or Prisma ran. Fixed with an explicit `SecurityEventAttackType` union/allow-list shared through the backend parser and repository filter, plus the matching frontend API type. Regression coverage now proves RATE_LIMIT, SQL_INJECTION, and XSS are accepted, arbitrary attack types remain `400`, and a RATE_LIMIT query returns only RATE_LIMIT events. Detection Analysis intentionally remains SQLi/XSS-only because RATE_LIMIT is not a Rule/ML classification.

## In Progress

**Generic upstream integration verification (2026-09-17):** A temporary, independently controlled Node HTTP container (not based on `protected-api`) was attached to `hybridwaf_default` and selected only by running the WAF with `UPSTREAM_URL=http://independent-upstream:3101`; no WAF source or compose file was changed. Through the WAF, `/hello?name=duytu`, JSON `/echo`, urlencoded `/echo`, multipart `/echo`, a real `302` redirect, and a 7-byte binary response all passed transparently. SQLi and XSS requests returned `403`; the independent upstream counter showed no corresponding `/hello` receipts. With temporary development limits `2 RPS / burst 5`, a controlled 20-request burst returned `4x200` and `16x429`, with `Retry-After: 1`; the origin counter remained at 6 `/hello` requests before the burst and did not increase for the 16 rejected requests. The resulting RATE_LIMIT persistence was verified through the existing database path. Stopping the independent upstream returned `502 Bad Gateway`. The normal bundled configuration was restored and temporary containers/processes were removed. The same-network topology was verified; for a real external origin, `UPSTREAM_URL=https://origin.example.com` changes the target, but firewall/security-group allow-listing is still required to prevent direct origin bypass.

**Runtime Rate Limiter verification (2026-09-17):** Docker wiring was audited and confirmed: `@UseGuards(RateLimitGuard)` is on `WafController`, the catch-all route is registered, `RateLimitModule` exports the guard and recorder, and default-scoped providers preserve bucket state. The normal container had no `RATE_LIMIT_*`, `WAF_MODE`, or `MAX_CONCURRENT_REQUESTS` overrides, so effective defaults were per-IP `20 RPS / burst 40`, global `200 RPS / burst 400`, concurrency `100`, and `HYBRID_BLOCK` mode. A controlled temporary runtime with per-IP `2 RPS / burst 5` produced 5 HTTP 200 responses and 15 HTTP 429 responses, all sharing normalized key `172.20.0.1` from raw `::ffff:172.20.0.1`; `Retry-After: 1` was present. Persistence verified `rateLimitBlocks = 284`, 284 `RATE_LIMIT` SecurityEvents, Admin Events `total: 284`, and Admin Stats `rateLimitBlocks: 284`. The initial 80-request all-200 result was expected under the default refill behavior and did not prove a defect. The original stale Docker image also retained the pre-fix admin allow-list; rebuilding from current source corrected that runtime contract.

**Detection analytics boundary fix (2026-09-17):** The live check showed `GET /admin/detection-analysis` was counting synthetic RATE_LIMIT events as Rule detections because `findBlockedInRange()` selected every BLOCK event. It now explicitly selects only `SQL_INJECTION` and `XSS`; RATE_LIMIT remains available to events/stats/dashboard attack distribution but is excluded from Rule-vs-ML analysis.

**Runtime upstream configuration and Settings UI (2026-09-18):** Added a persisted singleton `WafConfiguration` row and `UpstreamConfigService`. Active upstream precedence is runtime DB value, then `UPSTREAM_URL`, then `PROTECTED_API_URL`, then the existing localhost fallback. Authenticated `GET/PUT /admin/upstream` validates only absolute `http`/`https` URLs without embedded credentials, performs a bounded connectivity check before upsert, preserves the prior configuration on failure, and updates the in-process cache immediately on success. `UpstreamProxyService` and system status now resolve through this service, so subsequent requests use the runtime-selected target without restart. Settings was redesigned around the supplied mockup: a primary target-application card with integrated connection state, check-and-save action, open-website action, and supporting protection/rate-limit/quick-guide cards. Prisma migration `20260918090000_add_waf_configuration` was applied in Docker. Backend unit `264/264`, e2e `62/62`, build/lint pass; frontend build pass and lint has only the pre-existing `TrendingUp` warning in `lib/nav.ts`. Runtime route registration and `GET /admin/upstream` were verified after a forced backend image rebuild.

(none — P0 through P8 of the Reverse-Proxy Generalization & L7 Protection initiative are complete and fully verified; awaiting the user's own review/commit, per `docs/CLAUDE.md` §14)

## Next

- User review of the full P1-P8 diff (nothing committed — `git status`/`git diff` on the working tree shows everything). Once reviewed, the user commits/pushes themselves.
- **Explicitly not built, by design, not oversight:** Phase P4 (Redis-backed distributed rate limiting — deferred until the backend actually runs more than one instance, per ADR-9) and Phase P9 (DB-backed, dashboard-mutable settings — deferred as a separately-scoped, larger feature per the approved plan). Neither should be started without the user raising it again.
- For a real production capacity claim (as opposed to this session's local, same-machine, non-isolated comparison), re-run `load-testing/capacity-step.js` on the actual VPS with `ml-service`'s `WORKERS` tuned to its real core count.
- The user still needs to add the GitHub Actions repo secrets (`VPS_HOST`, `VPS_USERNAME`, `VPS_PASSWORD`, optionally `VPS_PORT`) before `.github/workflows/deploy.yml`'s `deploy` job can succeed — unrelated to this initiative, still outstanding from before it started.
- Separately, still open: the `/reports` orphaned route (exists, no nav entry), a restricted deploy-only VPS user to replace the root-password CI trade-off — neither part of this initiative, needs its own explicit scoping if picked up.
...
- ADR-8: Generic, configurable upstream (`UPSTREAM_URL`, migrated from `PROTECTED_API_URL`) — target is exclusively server-side env-config, never request-derived (SSRF/open-proxy invariant) — Approved, implemented Phase P1
- ADR-9: Layer-7 rate limiting, in-memory token bucket (per-IP + global) — Redis deferred until the backend is actually horizontally scaled, then fail-open with local fallback, not fail-closed; volumetric L3/4 DDoS stays a permanent non-goal — Approved, implemented Phase P3/P3a (Redis/P4 still deferred, as designed)
- ADR-10: ML onboarding modes (`MONITOR`/`RULE_BLOCK_ML_MONITOR`/`HYBRID_BLOCK`) as an additive parameter into the existing `HybridDecisionEngine.decide()` — Approved, implemented Phase P5
