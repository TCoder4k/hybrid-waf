...
**Not attempted:** actually SSHing in and running the deploy myself — the session's local SSH key (`~/.ssh/id_ed25519`) was rejected by the VPS for every username tried (root, ubuntu, admin, duytu, deploy, waf, hybridwaf, tu); the user has separate working credentials (confirmed logging in themselves as `root`) that aren't available in this environment. The user runs `./deploy.sh` on the VPS themselves; once `docker-compose.prod.yml` is committed/pushed and pulled onto the VPS, the VPS's own `docker-compose.yml` local-edit diff becomes redundant with the new file and should be discarded there (`git checkout -- docker-compose.yml`) before the next `./deploy.sh` run.
