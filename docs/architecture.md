...
`./deploy.sh` (repo root) is shorthand for the sequence a deploy always was: checks the VPS working tree has no uncommitted changes to tracked files (fails loudly rather than risk `git pull` silently clobbering an un-committed hotfix), `git pull origin main`, then `docker compose -f docker-compose.prod.yml up -d --build` (auto-detected — falls back to the plain dev file if `docker-compose.prod.yml` is absent from a given checkout), then prunes dangling images and prints container status.

**CI/CD** (`.github/workflows/deploy.yml`) runs `deploy.sh` automatically instead of by hand:
